...
namespace{
  G4Mutex TabulatedField3DLock = G4MUTEX_INITIALIZER;
}

using namespace std;

TabulatedField3D::TabulatedField3D(const char* filename,
						 double zOffset )
  :fZoffset(zOffset),invertX(false),invertY(false),invertZ(false)
{
  ...
  G4AutoLock lock(&TabulatedField3DLock);
  ...
  if (!file.is_open())
    {
      ...
      G4Exception("TabulatedField3D::TabulatedField3D",
		  "mag001",FatalException,ed);
    }
